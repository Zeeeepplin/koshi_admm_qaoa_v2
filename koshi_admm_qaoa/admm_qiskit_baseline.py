...
def build_admm_qp(net):
    ...
    heuristic topology-coupled objective on the binary switches + one continuous
...
def run_qiskit_admm(net=None, use_qaoa=False, maxiter=6):
    ...
    # same exact connected-radial projection used for the hand-rolled loop
    import power_model as pm
    from qubo_builder import z_to_dict
    repair = pm.radiality_repair(net, z_to_dict(x, meta))
    zd = repair["z"]
    ...
    print(f"  raw z={x}  -> projected z={xr}; actions="
          f"open {repair['opened_switches']}, close {repair['closed_switches']}")
    print(f"  validation (repaired): connected={val['connected']} "
          f"SOC loss={val['loss_mw']:.3f}MW shed={val['shed_mw']:.2f}MW "
          f"nonlinear AC validated={val['nonlinear_ac_validated']}")
    return res, xr
